          ...
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              ...
              <button onClick={isLastStep ? handleComplete : handleNext} className="next-button" {...nextButtonProps}>
                {isLastStep ? 'Concluir Etapas' : nextButtonText}
              </button>
            </div>
          </div>
        )}
...
function StepContentWrapper({
  ...
  return (
    <motion.div
      ...
      animate={{ height: isCompleted && !completedContent ? 0 : parentHeight || 'auto' }}
...
function StepIndicator({
  ...
  return (
    ...
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#18181D', color: '#9CA3AF' },
          active: { scale: 1.05, backgroundColor: '#D4AF37', color: '#0A0A0C' },
          complete: { scale: 1, backgroundColor: '#D4AF37', color: '#0A0A0C' }
        }}
